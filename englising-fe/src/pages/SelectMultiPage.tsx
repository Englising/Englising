...
const SelectMultiPage: React.FC = () => {
  ...
  const RoomButton: React.FC<RoomButtonProps> = ({ buttonText, apiEndpoint, onClick, selected }) => {
    return (
      <button
        className={`text-black ${selected ? "bg-secondary-500" : "bg-primary-500"} w-24 h-7 rounded-full text-sm hover:opacity-50`}
        onClick={() => onClick(apiEndpoint)}
      >
        {buttonText}
      </button>
    );
  ...
  return (
    <div className="bg-black relative w-[64rem] h-full py-8 flex flex-col mx-auto">
      {/* 대기방 목록 */}
      ...
      <div className="flex flex-row gap-6 pb-6">
        ...
        <RoomButton
          ...
          apiEndpoint="dance"
          onClick={handleClick}
          selected={selectedButton === "dance"}
        />
      </div>
      {multiroom && multiroom.length > 0 ? ( // multiroom 배열이 비어있지 않은 경우에만 map 함수 호출
        <div className="text-white grid grid-cols-4 gap-9 justify-items-start overflow-y-scroll scrollbar-webkit">
          {multiroom.map((item) => (
            <MultiRoom
              key={item.multiPlayId}
              roomName={item.roomName}
              roomId={item.multiPlayId}
              maxUser={item.maxUser}
              currentUser={item.currentUser}
              multiPlayImgUrl={item.multiPlayImgUrl}
