...
def get_album_dto_by_spotify_id(spotify_id: str) -> AlbumDto:
    log(LogList.SPOTIFY.name, LogKind.INFO, "Getting album {id}".format(id=spotify_id))
    try:
        result = spotify.album(spotify_id)
        return AlbumDto(
...
def search_track(track_name: str, artist_name: str):
    ...
    if tracks:
        track = tracks[0]
        track_info['track_spotify_id'] = track['id']
        track_info['album_spotify_id'] = track['album']['id']
        track_info['artist_spotify_id'] = track['artists'][0]['id']
    return track_info